...
"maximum_total_exceeded": f"Invalid input: total pages can not exceed {MAX_TOTAL_PAGES}",
"boundaries_around_exceeded": f"Invalid input: boundaries and around can not exceed {MAX_BOUNDARIES_AROUND}",
...
def _validate_input(current_page, total_pages, boundaries, around):
	...
	if current_page not in range(1, total_pages + 1):
		raise ValueError(ERROR_MESSAGES["current_outside_range"].format(current_page = current_page, total_pages = total_pages))
